...
export default function POSPage() {
  ...
  return (
    <div className="flex h-full" dir={dir}>
          ...
          {filtered.map(product => {
            ...
            return (
              <button
                ...
                disabled={outOfStock}
                className={`group p-4 rounded-xl border transition-all duration-200 ${dir === "rtl" ? "text-right" : "text-left"} ${
                  outOfStock
              ...
              >
                ...
                <p className="text-lg font-bold text-primary mt-2.5">{fmt(product.salePrice)}</p>
                <div className={`text-[11px] mt-1.5 font-semibold ${outOfStock ? "text-destructive" : lowStock ? "text-warning" : "text-success"}`}>
      ...
      {/* Cart Sidebar */}
      <div className={`w-80 lg:w-[360px] bg-card flex flex-col ${dir === "rtl" ? "border-l" : "border-r"} border-border`}>
            ...
            <div key={item.product.id} className="bg-muted/50 rounded-xl p-3 animate-scale-in border border-border/50">
              ...
              <div className="flex items-center justify-between mt-2.5">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => updateQuantity(item.product.id, -1)} className="w-7 h-7 rounded-lg bg-secondary flex items-center justify-center hover:bg-secondary/80 text-secondary-foreground transition-colors">
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-border space-y-2.5 bg-muted/30">
