...
import { User, Camera, Save, Loader2, AtSign } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast.error(isAr ? "حجم الصورة يجب أن لا يتجاوز 2MB" : "Image must be under 2MB");
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (error) {
      if (error.code === "23505") {
        toast.error(isAr ? "اسم المستخدم محجوز" : "Username already taken");
      } else {
  ...
  return (
    <div className="p-6 max-w-lg mx-auto" dir={dir}>
      ...
      <div className="bg-card border border-border rounded-2xl p-6 space-y-6">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-muted-foreground">{isAr ? "اضغط لتغيير الصورة" : "Click to change photo"}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {isAr ? "أحرف إنجليزية صغيرة وأرقام و _ فقط (3-30 حرف)" : "Lowercase letters, numbers, and _ only (3-30 chars)"}
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm border border-border rounded-lg bg-muted px-3 py-2.5 text-muted-foreground">
            {user?.email}
          </div>
        </div>

        <Button onClick={handleSave} className="w-full touch-target gap-2" size="lg" disabled={loading}>
